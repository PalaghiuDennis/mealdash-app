...
?>
  ...
  <!-- ======= Hero Section ======= -->
  <section id="comenzi" class="comenzi d-flex align-items-center section-bg">
      <div class="container" data-aos="fade-up">
        <div class="section-header">
          <p>Comenzile<span>Tale</span></p>
    </div>
    </div>
        <?php
include("include/connection.php");

// Citirea adresei de email din fișierul "email.txt"
$filename = "email.txt";
...
if (!empty($emails)) {
    ...
    // Obține clientID-ul utilizatorului din baza de date
    $query = "SELECT clientID FROM clienti WHERE email = '$email'";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $query_comenzi = "SELECT * FROM comenzi WHERE clientID = '$clientID'";
        ...
        if ($result_comenzi && mysqli_num_rows($result_comenzi) > 0) {
            ...
            echo '<table>
                    <tr>
                        <th><h2>Nr.</h2></th>
                        <th><h2>Pret Total</h2></th>
                        <th><h2>Nume</h2></th>
                        <th><h2>Adresa</h2></th>
                        <th><h2>Telefon</h2></th>
                        <th><h2>Data Trimiteri</h2></th>
                        <th><h2></h2></th>
            ...
            while ($row_comenzi = mysqli_fetch_assoc($result_comenzi)) {
                ...
                echo '<td><h3>' . $row_comenzi['pret_total'] . '</h3></td>';
                echo '<td><h3>' . $row_comenzi['nume'] . '</h3></td>';
                echo '<td><h3>' . $row_comenzi['adresa'] . '</h3></td>';
                echo '<td><h3>' . $row_comenzi['telefon'] . '</h3></td>';
                echo '<td><h3>' . $row_comenzi['data_trimiteri'] . '</h3></td><h3>';
                
                echo '<td><form action="pagina_detalii_comenzi.php" method="POST">';
                echo '<input type="hidden" name="comanda" value="' . $comanda . '">';
                echo '<input type="submit" value="Detalii">';
                echo '</form></td>';  
                echo '</h3></tr>';
                $numar=$numar+1;
            }
